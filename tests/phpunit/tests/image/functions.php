...
class Tests_Image_Functions extends WP_UnitTestCase {
	...
	public function test_pdf_preview_doesnt_overwrite_existing_jpeg() {
		...
		// Dummy JPEGs.
		$jpg1_path = get_temp_dir() . 'test.jpg'; // Straight.
		file_put_contents( $jpg1_path, 'asdf' );
		$jpg2_path = get_temp_dir() . 'test-pdf.jpg'; // With PDF marker.
		file_put_contents( $jpg2_path, 'fdsa' );

		// PDF with same name as JPEG.
		$pdf_path = get_temp_dir() . 'test.pdf';
		...
		$preview_path = get_temp_dir() . '' . $metadata['sizes']['full']['file'];
